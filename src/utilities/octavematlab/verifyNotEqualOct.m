function structtest = verifyNotEqualOct(testCase,actual,expected)
    % TODO: doc
if ~isOctave && ~verLessThan('matlab','8.1') % function introduced R2013a    
    verifyNotEqual(testCase,actual,expected);
else
    structtest=-1;
...
try
    ...
    structtest=0; % if comparisons fail then they are not equal
end
end
