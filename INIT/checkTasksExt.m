function [taskReport, essentialRxns, taskStructure, essentialFluxes]=checkTasksExt(model,inputFile,printOutput,printOnlyFailed,getEssential,taskStructure)
%This has been extended by adding the essentialFluxes. The purpose of this is to determine
%in which direction the essential rxn should go - if it is in the direction
%returned, it works. We can then in ftINIT, if essential rxns have the same 
%direction for all tasks, force the flux on in that direction, and thereby reduce the
%problem size (compared to forcing it on in any direction, which requires an integer 
%in the MILP).
%
% checkTasksExt
