...
if any(I)
    if isempty(rxns)
        dispEM(['The following equation does not have reversibility data: ' equations{I} ]);
    else
        dispEM(['The reaction ' rxns{I} ' does not have reversibility data']);
    end
end
...
%Replace the the plus signs with a weird character that will be used for
%parsing
equations=strrep(equations,' + ', '¤');
