...
for i=1:length(v.model{1,1}.annotation{1,1}.listOfSpeciesAliases{1,1}.speciesAlias)
    ...
    for j=1:length(v.model{1,1}.listOfSpecies{1,1}.species)
        if strcmpi(v.model{1,1}.listOfSpecies{1,1}.species{1,j}.ATTRIBUTE.id,...
            ...
            %visualization of fluxes) by making a note in Celldesigner for
            %the protein in interest (this can only be done for proteins at
            %the moment). The note (not protein note!) should only contain
            %the reaction id in the model which corresponds to the protein
            %in Celldesigner.
...
for i=1:length(v.model{1,1}.listOfReactions{1,1}.reaction)
    ...
    %Adds the non-base reactants Not all reactions have any non-base
    %reactants
    ...
    %Adds the non-base products Not all reactions have any non-base
    %products
    if isfield(v.model{1,1}.listOfReactions{1,1}.reaction{1,i}.annotation{1,1}...
            ,'listOfProductLinks')
        for j=1:length(v.model{1,1}.listOfReactions{1,1}.reaction{1,i}.annotation{1,1}...
    ...
    %Adds the modifiers (enzymes) Not all reactions have any modifiers
    if isfield(v.model{1,1}.listOfReactions{1,1}.reaction{1,i}.annotation{1,1}...
            ,'listOfModification')
        for j=1:length(v.model{1,1}.listOfReactions{1,1}.reaction{1,i}.annotation{1,1}...
...
%   speciesAlias    the alias of the species bindingSite     The binding
%   site in Celldesigner is a string of up to
%                   three letters that defines which of the 16 bindings
%                   sites on each species to use. They are written as N
%                   (north), NNW (north north west) and so on
%
...
for i=1:length(pathway.listOfSpecies)
    if strcmpi(pathway.listOfSpecies(i).alias,speciesAlias)
        ...
        %Species marked as "PROTEIN" should be drawn as rectangles. There
        %is one binding site in each corner and three in between on each
        %side.
        if strcmpi(pathway.listOfSpecies(i).type,'PROTEIN')
