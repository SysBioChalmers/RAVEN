function [rxnScores, geneScores, hpaScores, arrayScores] = scoreModel(model,hpaData,arrayData,tissue,varargin)
% scoreModel  Score reactions and genes in a model with complex gene rules.
%
...
% as min(5*log(x),10) for x > 1 and max(5*log(x),-5) for x < 1, to
