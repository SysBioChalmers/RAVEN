...
%pathway databases. A combined model with the comprehensive coverage of
...
%whole-proteome sequences. The combined model will be subsequently used for
%refinement of existing high-quality model and generation of a new version
%of GEM, by utilizing the manual curation results. This tutorial is a
%showcase of the new features released in RAVEN 2.0 through demonstrating
%the utilization of the newly developed functions on GEM reconstruction and
%curation for Streptomyces coelicolor strain A3(2). Users may apply this
%approach as the template in their own work for other organisms.
%
% Simonas Marcisauskas, 2019-09-13
%

%Note that the detailed instructions of the RAVEN functions can be found
%both in Matlab by typing 'help functionName' and from opening the HTML
...
%parameter is organismID and needs to be specified by user. The other
%parameters are set to exclude unbalanced and undetermined reactions, but
%keep transport rections. The two parameters for homology search are set to
%the default values that have been optimized to capture the protein hits
%with both comprehensive coverage and the least false positives.
...
%is reconstructed from the genome information annotated by KEGG. Since KEGG
...
%and MetaCyc databases are formulated in different ways; KEGG relys on
