...
% TIMER callback changes the current directory. Only canonical paths
% without "." and ".." can be recognized uniquely. Long path names (>259
% characters) require a magic initial key "\\?\" to be handled by Windows
% API functions, e.g. for Matlab's FOPEN, DIR and EXIST.
%
% Parameters
% ----------
% File : char or cell
...
% -------
% File : char or cell
%     absolute canonical path name as a string or cell string. For empty
%     strings the current directory is replied. '\\?\' or '\\?\UNC' is
%     added on demand.
%
% Examples
% --------
...
% Notes
% -----
% The M- and the MEX-version create the same results, the faster MEX
% function works under Windows only. Some functions of the Windows-API
% still do not support long file names. E.g. the Recycler and the Windows
% Explorer fail even with the magic '\\?\' prefix. Some functions of Matlab
% accept 260 characters (value of MAX_PATH), some at 259 already. Don't
% blame me. The 'fat' style is useful e.g. when Matlab's DIR command is
% called for a folder with less than 260 characters, but together with the
% file name this limit is exceeded. Then "dir(getFullPath([folder,
% '\*.*], 'fat'))" helps.
%
% COMPILE:
...
if isempty(hasDataRead)
    % Test this once only - there is no relation to the existence of
    % DATAREAD!
    %if isWIN
    %   Show a warning, if the slower Matlab version is used - commented,
    %   because this is not a problem and it might be even useful when the
    %   MEX-folder is not inlcuded in the path yet.
    ...
    % DATAREAD is deprecated in 2011b, but still available. In Matlab 6.5,
    % REGEXP does not know the 'split' command, therefore DATAREAD is
    % preferred:
...
if isWIN  % Windows: --------------------------------------------------------
    ...
    if isUNC == 0                        % File is not a UNC path
        % Leading file separator means relative to current drive or base
        % folder:
        ThePath = cd;
        if File(1) == FSep
        ...
        elseif FileLen == 2 && File(2) == ':'   % "C:" current directory on C!
            % "C:" is the current directory on the C-disk, even if the
            % current directory is on another disk! This was ignored in
            % Matlab 6.5, but modern versions considers this strange
            % behaviour.
...
if isWIN
    ...
    if (strncmpi(Style, 'a', 1) && length(File) >= MAX_PATH) || ...
            strncmpi(Style, 'f', 1)
        % Do not use [isUNC] here, because this concerns the input, which
        % can '.\File', while the current directory is an UNC path.
        if strncmp(File, '\\', 2)  % UNC path
            File = ['\\?\UNC', File(2:end)];
        else
            File = ['\\?\', File];
