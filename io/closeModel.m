...
for i=1:numel(closedModel.rxns)
    if sum(abs(closedModel.S(:,i)))==1
        closedModel.mets{numel(closedModel.mets)+1}=strcat(closedModel.mets{closedModel.S(:,i)},'_b');
        if isfield(closedModel,'metNames')
            closedModel.metNames{numel(closedModel.metNames)+1}=closedModel.metNames{closedModel.S(:,i)};
        end
        closedModel.metComps(numel(closedModel.metComps)+1)=numel(closedModel.comps);
        if isfield(closedModel,'inchis')
            closedModel.inchis{numel(closedModel.inchis)+1}=closedModel.inchis{closedModel.S(:,i)};
        end
        if isfield(closedModel,'metFormulas')
            closedModel.metFormulas{numel(closedModel.metFormulas)+1}=closedModel.metFormulas{closedModel.S(:,i)};
        end
        if isfield(closedModel,'metMiriams')
            closedModel.metMiriams{numel(closedModel.metMiriams)+1}=closedModel.metMiriams{closedModel.S(:,i)};
        end
        if isfield(closedModel,'metFrom')
            closedModel.metFrom{numel(closedModel.metFrom)+1}='';
        end
        if isfield(closedModel,'metCharges')
            closedModel.metCharges(numel(closedModel.metCharges)+1)=closedModel.metCharges(closedModel.S(:,i));
        end
