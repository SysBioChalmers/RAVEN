...
% drawMap  Plot fluxes on a previously drawn metabolic network map.
%
% Imports a previously drawn map of the metabolic network and plots the
% fluxes on that map. If the pathway contains expression data, the
% log-fold changes are plotted as well.
%
% Parameters
% ----------
% title : char
%     title to be printed on the map.
% pathway : struct
%     map structure as generated by constructPathwayFromCelldesigner.
% modelA : struct
%     model structure for condition A.
% conditionA : double
%     flux vector for condition A.
% conditionB : double, optional
%     flux vector for condition B (default conditionA).
% modelB : struct, optional
%     model structure for condition B (default modelA).
% filename : char, optional
%     if provided, exports the map as a pdf to this file.
% cutOff : double, optional
%     only print fluxes where one of the fluxes is above this value
%     (default 10^-7).
% supressOpen : logical, optional
%     true if the pdf file should not be opened (default false).
%
% Returns
% -------
% notMapped : cell
%     the reaction ids that carried a flux in either of the conditions but
%     that were not present in the metabolic map.
%
% Examples
% --------
%     notMapped = drawMap(title, pathway, modelA, conditionA, ...
