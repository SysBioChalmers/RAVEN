...
% tAnnotation  Tests for the annotation/metadata functions in annotation/.
    ...
    methods (Test)
        ...
        function deltaGCsvRoundTrip(testCase)
            ...
            evalc('saveDeltaGCSV(m, metCsv, rxnCsv);');
            base = testCase.model;
            evalc('m2 = loadDeltaGCSV(base, metCsv, rxnCsv);');
        ...
        function loadDeltaGCsvStoresValuesLiterally(testCase)
            ...
            fclose(fid);
            evalc('md = loadDeltaGCSV(m, ''metCsv'', metCsv);');
            testCase.verifyEqual(md.metDeltaG(1), -42.5, 'AbsTol', 1e-9);
            testCase.verifyEqual(md.metDeltaG(2), 1e7, 'AbsTol', 1e-3);
        end
