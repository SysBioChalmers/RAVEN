...
if ~testGurobi
    disp('Gurobi not installed or not functional, some mergeModelsTests skipped.')
    skipTests = contains({tests.Name},'gurobi','IgnoreCase',true);
    tests(skipTests) = [];
end
if exist('scip','file')~=3
    disp('SCIP MEX binary not installed or not functional, some mergeModelsTests skipped.')
...
function mergeModelsViaFillGapsScipTest(testCase)
%fillGaps merges the model with the template(s) through mergeModels and relies
%on the rxnFrom field to track which reactions originate from the model. Check
%that gap-filling works and that the resulting From fields stay consistent
sourceDir = fileparts(which(mfilename));
load([sourceDir,'/test_data/ecoli_textbook.mat'], 'model');
...
%Remove first 10 reactions to create gaps
modelGaps=removeReactions(modelDB,(1:10));
modelDB.id='DB';
try
    evalc('[newConnected,cannotConnect,addedRxns,newModel,exitFlag]=fillGaps(modelGaps,modelDB)');
catch
    try
...
%The gap-filled model should be able to carry flux
verifyTrue(testCase,sol.f>0);
%fillGaps merges via mergeModels; the resulting From fields must stay
%consistent in size with the returned model
verifyEqual(testCase,numel(newModel.rxnFrom),numel(newModel.rxns));
verifyEqual(testCase,numel(newModel.metFrom),numel(newModel.mets));
end

function mergeModelsViaFillGapsGurobiTest(testCase)
%fillGaps merges the model with the template(s) through mergeModels and relies
%on the rxnFrom field to track which reactions originate from the model. Check
%that gap-filling works and that the resulting From fields stay consistent
sourceDir = fileparts(which(mfilename));
