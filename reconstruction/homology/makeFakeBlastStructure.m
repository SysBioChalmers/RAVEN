...
%
% This is a structure that would normally be generated by getBlast. It
% allows to feed a predefined list of orthologs to getModelFromHomology
% while retaining the further use of that function. For this function to
% work, it is crucial that the orthologList is a cell array where the first
% column contains the genes from the source organism, and the second column
% contains the genes from the target organism.
%
% Parameters
% ----------
% orthologList : cell
%     cell array of orthologous genes, where the first column contains the
%     genes from the source organism, while the second column contains the
%     genes from the target organism.
% sourceModelID : char
%     ID of the model that will be used as template, that contains the
%     genes in the first column of orthologList.
% getModelFor : char
%     the name of the organism to build a model for, identical to the
%     getModelFor parameter in the getModelFromHomology function.
%
% Returns
% -------
% blastStructure : struct
%     a fake blastStructure, where the evalue, identity and aligLen are
%     set at extreme values, such that all orthologous pairs will pass the
%     filter when running getModelFromHomology.
%
% Examples
% --------
